...
////// Begin Check for news from integramod
   ...
   {
      @fputs($fsock, "GET /forum/updateNews/im3_news_3.7b.txt HTTP/1.1\r\n");
   ...
   else
   {
      if ($errstr)
      {
         $news = '<p style="color:red">' . sprintf($user->lang['CONNECT_SOCKET_ERROR_INTEGRA'], $errstr) . '</p>';
      }
      else
      {
	...
	$template->assign_vars(array('INTEGRA_NEWS' => $news,));
