...
*
* @package calendar
* @version $Id: calendar.$phpEx 2008-07-10 21:16:00Z livewirestu and NeXur $
...
// Note this file is included after user->setup(), to ensure that some globals in functions_calendar.$phpEx can
// find the $user->lang array
include($phpbb_root_path . 'includes/functions_calendar.' . $phpEx);
$user_id = $user->data['user_id'];

// Permissions
$can_view = $auth->acl_get('u_view_event');
$can_post_new = $auth->acl_get('u_new_event');
$can_edit_self = $auth->acl_get('u_edit_event');
$can_delete_self = $auth->acl_get('u_delete_event');
...
switch($mode)
{
	...
	case 'edit':
		if (!($can_edit_others || ($can_edit_self && (get_poster_id(request_var('event_id', '')) == $user_id))))
			$authed = false;
	break;
...
switch($mode)
{
	...
	/******* BEGIN Stuff which applies to all conditions ********/
		...
		$template->assign_vars(array(
			...
			'NUM_GROUPS_18'				=> 18*sizeof($user_groups),
			...
			if($mode == 'edit')
			{
				...
				if(empty($error))
				{
					...
					$template->assign_vars(array(
						...
						'PRIV_USER_LIST'                        => $event_data['priv_users'] ? $event_data['priv_users'] : '',
	...
	/************************* END Mode specific stuff ******************************/
		...
		// Spit it out
		$page_title = $user->lang['LANG_CALENDAR'];
	...
	case 'attend':
		...
		if(!empty($tid))
		{
			$message .= '<br /><br />' . $user->lang['RETURN_EVENT_TOPIC'];
			$meta_url = append_sid("{$phpbb_root_path}viewtopic.php", "f=$fid&amp;t=$tid");
		}
		else
		{
		...
		*/
		if(!empty($tid))
		{
			$message .= ($mode != 'attend') ? sprintf($user->lang['RETURN_TOPIC'], '<br /><br /><a href="' . $meta_info . '">', '</a>') : '';
			$meta_url = append_sid("{$phpbb_root_path}viewtopic.php", "f=$fid&amp;t=$tid");
		}
		else
		{
			$message .= ($mode != 'attend') ? sprintf($user->lang['RETURN_CALENDAR'], '<br /><br /><a href="' . $meta_info . '">', '</a>') : '';
			$meta_url = append_sid("{$phpbb_root_path}calendar.$phpEx", "mode=view&amp;month=$m&amp;day=$d&amp;year=$y");
		}
		meta_refresh(3, $meta_url);
		trigger_error($message);
	...
	case 'view':    // Viewing events of a given day.
				...
				foreach ($events_array as $row)
				{
					...
					$template->assign_block_vars('event_row', array(
						...
						'U_DELETE_ACTION'       => append_sid("{$phpbb_root_path}calendar.$phpEx", "mode=delete&amp;event_id=" . $row['event_id']),
                      	...
                      	'S_EDIT'                => ($row['topic_link'] == null && (($auth->acl_get('u_edit_event') && $row['user_id'] == $user->data['user_id']) || $auth->acl_get('m_edit_event') || $auth->acl_get('a_edit_event'))) ? true : false,
                      	'S_DELETE'              => ($row['topic_link'] == null && (($auth->acl_get('u_delete_event') && $row['user_id'] == $user->data['user_id']) || $auth->acl_get('m_delete_event') || $auth->acl_get('a_delete_event'))) ? true : false,
		...
		{
			header('Location:calendar.$phpEx');
		}
	...
	case 'delete':
		...
		if(!($can_delete_others || ($can_delete_self && (get_poster_id(request_var('event_id', '')) == $user_id))))
		{
			$message = $user->lang['NO_AUTH_OPERATION'] . '<br /><br />' . sprintf($user->lang['RETURN_INDEX'], '<a href="' . $phpbb_root_path . '">', '</a>');
...
function build_calendar_panel()
{
			...
			if( (sizeof($event_array) > 0) || ((sizeof($bday_array) > 0) && $s_show_birthdays) )
			{
				...
				foreach($event_array as $this_event)
				{
					...
					if($todays_event_count <= 4)
					{
						...
						$output_text .= '<a href="'.append_sid("{$phpbb_root_path}calendar.$phpEx", 'mode=view&month=' . $cal_month . '&day=' . $week[$i] . '&year=' . $cal_year) . '"  title="' . $user->lang['STARTS'] . ' ' . gmdate($user->date_format, $this_event['start_time']) . ", " . $user->lang['ENDS'] . ' ' . gmdate($user->date_format, $this_event['end_time']) . '">' . substr($this_event['event_name'], 0, 15) . (strlen($this_event['event_name'])>15?'...':'') . '</a><br>';
					}
					else
					{
						if(sizeof($event_array) > 4)
						{
							$output_text .= '<a href="'.append_sid("{$phpbb_root_path}calendar.$phpEx", 'mode=view&month=' . $cal_month . '&day=' . $week[$i] . '&year=' . $cal_year) . '"  title="' . $user->lang['MORE_EVENTS'] . '">' . $user->lang['MORE_EVENTS'] . '</a><br>';
							break;
						}
						else
						{
				...
				if(sizeof($bday_array) > 0 && $s_show_birthdays)
				{
					...
					{
						$output_text .= '<a href="'.append_sid("{$phpbb_root_path}calendar.$phpEx", 'mode=view&month=' . $cal_month . '&day=' . $week[$i] . '&year=' . $cal_year) . '" title="Birthday" >' . $user->lang['BIRTHDAYS'] . '</a><br>';
					}
					else
					{
						$output_text .= '<a href="'.append_sid("{$phpbb_root_path}calendar.$phpEx", 'mode=view&month=' . $cal_month . '&day=' . $week[$i] . '&year=' . $cal_year) . '" title="Birthday" >' . $bday_array[0]['name'] . ' (' . $bday_array[0]['age'] . ')</a><br>';
					}
				}
				else
				{
				...
				$week[$i] = array(
					'day'       => '<b><a href="'.append_sid("{$phpbb_root_path}calendar.$phpEx", 'mode=view&month=' . $cal_month.'&day=' . $week[$i] . '&year=' . $cal_year) . '">' . $week[$i] . '</a></b>',
					'event'     => $output_text,
				);
			}
			else
	...
	$template->assign_vars(array(
		...
		'L_CALENDAR_MONTH'     => $user->lang['CALENDAR']['LONG_MONTH'][(int)$cal_month] . " " . $cal_year,
...
function get_poster_id($event_id)
{
  global $db;
   //get the event originator
  $sql = 'SELECT user_id FROM ' . CALENDAR_TABLE . ' WHERE event_id = ' . $event_id;
  $result = $db->sql_query($sql);
  $poster_id = $db->sql_fetchrow($result);
  $db->sql_freeresult($result);
