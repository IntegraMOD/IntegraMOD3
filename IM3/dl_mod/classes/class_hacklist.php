...
* @file				class_hacklist.php 3 2009/05/3026 OXPUS
	...
	*/
	function hacklist()
	{
		...
		*/
		...
		{
			$this->dl_config['use_hacklist'] = $row['config_value'];
		}
		...
		*/
		...
		if ($this->dl_config['use_hacklist'])
		{
	...
	function hacklist_status()
	{
		return $this->dl_config['use_hacklist'];
	}
	...
	function hacks_index()
	{
		...
		while ($row = $db->sql_fetchrow($result))
		{
			...
			if ($row['auth_view'] || (isset($this->dl_auth[$cat_id]['auth_view']) && $this->dl_auth[$cat_id]['auth_view']) || ($auth->acl_get('a_') && $user->data['is_registered']))
			{
