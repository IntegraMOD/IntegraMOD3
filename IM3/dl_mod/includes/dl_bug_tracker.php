...
* @file				dl_bug_tracker.php 6 2009/06/04 OXPUS
...
if ($action == 'detail' && $fav_id)
{
	...
	$template->assign_vars(array(
		'L_REPORT_ID'		=> $user->lang['DL_BUG_REPORT_ID'],
		'L_REPORT_TITLE'	=> $user->lang['DL_BUG_REPORT_TITLE'],
		'L_REPORT_TEXT'		=> $user->lang['DL_BUG_REPORT_TEXT'],
		'L_REPORT_DATE'		=> $user->lang['DL_BUG_REPORT_DATE'],
		'L_REPORT_PHP'		=> $user->lang['DL_BUG_REPORT_PHP'],
		'L_REPORT_DB'		=> $user->lang['DL_BUG_REPORT_DB'],
		'L_REPORT_FORUM'	=> $user->lang['DL_BUG_REPORT_FORUM'],
		'L_REPORT_FILE'		=> $user->lang['DL_BUG_REPORT_FILE'],
		'L_REPORT_FILE_VER'	=> $user->lang['DL_HACK_VERSION'],
		'L_REPORT_AUTHOR'	=> $user->lang['DL_BUG_REPORT_AUTHOR'],
		'L_STATUS'			=> $user->lang['DL_BUG_REPORT_STATUS'],
		'L_STATUS_DATE'		=> $user->lang['DL_BUG_REPORT_STATUS_DATE'],
		'L_STATUS_TEXT'		=> $user->lang['DL_BUG_REPORT_STATUS_TEXT'],
		'L_STATUS_UPDATE'	=> $user->lang['DL_BUG_REPORT_STATUS_UPDATE'],
		'L_REPORT_HISTORY'	=> $user->lang['DL_BUG_REPORT_HISTORY'],
		'L_NO_ASSIGN'		=> $user->lang['DL_BUG_REPORT_UNASSIGNED'],
		'L_ASSIGN_TO'		=> $user->lang['DL_BUG_REPORT_ASSIGNED'],
		'L_ASSIGN_DATE'		=> $user->lang['DL_BUG_REPORT_ASSIGN_DATE'],
		'L_SUBMIT'			=> $user->lang['DL_BUG_REPORT_ASSIGN'],

		'L_DOWNLOAD'		=> $user->lang['DL_CAT_TITLE'],
	...
	if ($total_history)
	{
		...
		while ($row = $db->sql_fetchrow($result))
		{
			...
			if ($report_his_type == 'assign')
			{
				...
				$output_text = $user->lang['DL_BUG_REPORT_ASSIGNED'].' -> <a href="'.append_sid("{$phpbb_root_path}memberlist.$phpEx?mode=viewprofile", "u=".$output_data[0]).'" target="_blank">'.$output_data[1].'</a>';
			}
			else if ($report_his_type == 'status')
			{
				...
				$output_text .= (isset($output_data[2])) ? '<hr />'.str_replace("\n", "<br />", $output_data[2]) : '';
			}
...
if ($action == 'add' && $user->data['is_registered'])
{
	...
	$template->assign_vars(array(
		...
		'L_DOWNLOAD'		=> $user->lang['DL_CAT_TITLE'],
		'L_BUG_TRACKER'		=> $user->lang['DL_BUG_TRACKER'],
		'L_ADD_REPORT'		=> $user->lang['NEW_POST'],
...
if ($action == 'delete' && $df_id && $fav_id && $allow_bug_mod)
{
	if (!$confirm && !$cancel)
	{
		...
		$template->assign_vars(array(
			...
			'MESSAGE_TEXT' => $user->lang['CONFIRM_DELETE_BUG_REPORT'],
...
if (!$action)
{
	...
	$template->assign_vars(array(
		'L_REPORT_TITLE'			=> $user->lang['DL_BUG_REPORT_TITLE_DETAILS'],
		'L_REPORT_TEXT'				=> $user->lang['DL_BUG_REPORT_TEXT'],
		'L_REPORT_DATE'				=> $user->lang['DL_BUG_REPORT_DATE'],
		'L_REPORT_PHP'				=> $user->lang['DL_BUG_REPORT_PHP'],
		'L_REPORT_DB'				=> $user->lang['DL_BUG_REPORT_DB'],
		...
		'L_REPORT_FILE'				=> $user->lang['DL_BUG_REPORT_FILE'],
		'L_REPORT_AUTHOR'			=> $user->lang['DL_BUG_REPORT_AUTHOR'],
		'L_REPORT_ASSIGN'			=> $user->lang['DL_BUG_REPORT_ASSIGNED'],
		'L_REPORT_ASSIGN_DATE'		=> $user->lang['DL_BUG_REPORT_ASSIGN_DATE'],
		'L_REPORT_STATUS'			=> $user->lang['DL_BUG_REPORT_STATUS'],
		'L_REPORT_STATUS_DATE'		=> $user->lang['DL_BUG_REPORT_STATUS_DATE'],
		'L_REPORT_DETAIL'			=> $user->lang['DL_BUG_REPORT_DETAIL'],
		'L_REPORT_FILTER'			=> $user->lang['DL_FILTER'],
		'L_REPORT_FILTER_OWN'		=> $user->lang['DL_FILTER_BT_OWN'],
		'L_REPORT_FILTER_ASSIGN'	=> $user->lang['DL_FILTER_BT_ASSIGN'],
		'L_DOWNLOAD'				=> $user->lang['DL_CAT_TITLE'],
		'L_BUG_TRACKER'				=> $user->lang['DL_BUG_TRACKER'],
		'L_NO_BUG_TRACKER'			=> $user->lang['DL_NO_BUG_TRACKER'],
		'L_OPTION'					=> $user->lang['OPTIONS'],
	...
	*/
	...
	{
		$pagination = generate_pagination(append_sid("{$phpbb_root_path}downloads.$phpEx?view=bug_tracker", "df_id=$df_id"), $total_reports, $dl_config['dl_links_per_page'], $start, true);
	}
	else
	{
		$pagination = '';
	...
	if ($reports_num)
	{
		while ($row = $db->sql_fetchrow($result))
		{
			...
			{
				$template->assign_block_vars('bug_tracker_row.no_assign', array(
					'L_NO_ASSIGNED' => $user->lang['DL_BUG_REPORT_UNASSIGNED'])
				);
			}
