...
* @file				dl_comments.php 5 2009/06/04 OXPUS
...
{
	$sql = 'SELECT description, desc_uid, desc_flags FROM ' . DOWNLOADS_TABLE . "
		WHERE cat = $cat_id
			AND id = $df_id";
	$result = $db->sql_query($sql);
	...
	$text_ary		= generate_text_for_edit($row['description'], $row['desc_uid'], $row['desc_flags']);
	...
	$template->assign_vars(array(
		'L_DL_COMMENT'		=> $user->lang['DL_COMMENT'],
		...
		'U_DL_LINK'			=> append_sid("{$phpbb_root_path}downloads.$phpEx?view=detail", "df_id=$df_id"),
	...
	*/
	...
	if ($total_comments)
	{
		...
		$template->assign_vars(array(
			...
			'L_POSTER'			=> $user->lang['USERNAME'],
			'L_MESSAGE'			=> $user->lang['DL_COMMENT'],
			'L_DL_DELETE'		=> $user->lang['DL_COMMENT_DELETE'],
			'L_DL_EDIT'			=> $user->lang['DL_COMMENT_EDIT'],
			'L_POST_COMMENT'	=> ($deny_post) ? '' : $user->lang['DL_COMMENT_WRITE'],
			'L_CAT_NAME'		=> $user->lang['DL_CAT_NAME'],
			'L_DOWNLOADS'		=> $user->lang['DOWNLOADS'],
			'L_DL_CATS'			=> $user->lang['DL_CAT_TITLE'],
			'L_GOTO_PAGE'		=> $user->lang['GOTO_PAGE'],
			'L_ACTION'			=> $user->lang['OPTIONS'],

			'ICON_EDIT'			=> $user->lang['EDIT_POST'],
			'ICON_DELETE'		=> $user->lang['DELETE_POST'],
