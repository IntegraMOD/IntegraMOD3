...
* @file				dl_footer.php 10 2009/09/02 OXPUS
...
if (sizeof($index) || $cat)
{
	...
	*/
	$dl_jumpbox = '<form method="post" name="dl_jumpbox" action="'.append_sid("{$phpbb_root_path}downloads.$phpEx?sort_by=$sort_by&amp;order=$order").'" onsubmit="if(this.options[this.selectedIndex].value == -1){ return false; }">';
	$dl_jumpbox .= $user->lang['JUMP_TO'] . ': <select name="cat" onchange="if(this.options[this.selectedIndex].value != -1){ forms[\'dl_jumpbox\'].submit() }">';
	...
	$dl_jumpbox .= '</select>&nbsp;<input type="submit" value="'.$user->lang['GO'].'" class="button2" /></form>';
	...
	*/
	...
	$template->assign_vars(array(
		'L_DL_BLUE_EXPLAIN'		=> $user->lang['DL_BLUE_EXPLAIN_FOOT'],
		'L_DL_GREEN_EXPLAIN'	=> $user->lang['DL_GREEN_EXPLAIN'],
		'L_DL_WHITE_EXPLAIN'	=> $user->lang['DL_WHITE_EXPLAIN'],
		'L_DL_GREY_EXPLAIN'		=> $user->lang['DL_GREY_EXPLAIN'],
		'L_DL_RED_EXPLAIN'		=> sprintf($user->lang['DL_RED_EXPLAIN'], $dl_config['dl_posts']),
		'L_DL_YELLOW_EXPLAIN'	=> $user->lang['DL_YELLOW_EXPLAIN'],
		'L_NEW_DL'				=> $user->lang['DL_NEW'],
		'L_EDIT_DL'				=> $user->lang['DL_EDIT'],
		'L_OVERALL_VIEW'		=> $user->lang['DL_OVERVIEW'],
		'L_CAN_DOWNLOAD_AGAIN'	=> $l_can_download_again,
		'L_TODOLIST'			=> $user->lang['DL_MOD_TODO'],
		'L_CONFIG'				=> $user->lang['DL_CONFIG'],
		...
		'IMG_RED'				=> $user->img('dl_red', sprintf($user->lang['DL_RED_EXPLAIN_ALT'], $dl_config['dl_posts'])),
	...
	if ($dl_config['show_footer_legend'] && (!($view == 'search' && $submit) && !(in_array($view, array('user_config', 'todo', 'stat', 'upload', 'comment', 'bug_tracker'))) || $cat))
	{
