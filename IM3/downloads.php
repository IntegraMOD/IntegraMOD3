...
* @file				downloads.php 26 2009/10/02 OXPUS
...
if ($view == 'code' && $code)
{
	include($phpbb_root_path . 'dl_mod/includes/dl_code.' . $phpEx);
}
else if($view == 'code' && !$code)
{
	exit;
}
...
if ((!$cat && (!$view || $view == 'view')) || $cat || !sizeof($_GET) || (sizeof($_GET) == 1 && isset($sid)))
{
	$enable_desc = true;
	$user->data['dl_enable_desc'] = true;
}
if ($cat)
{
	$enable_rule = true;
	$user->data['use_dl_enable_rule'] = true;
}
...
switch ($view)
{
	...
	case 'detail':
		...
		$nav_string['name'][] = $user->lang['DL_DETAIL'];
	break;
	case 'comment':
		$nav_string['link'][] = append_sid("{$phpbb_root_path}downloads.$phpEx?view=detail&amp;df_id=$df_id");
		$nav_string['name'][] = $user->lang['DL_DETAIL'];
...
*/
if ($view == 'broken' && $df_id && $cat_id && ($user->data['is_registered'] || (!$user->data['is_registered'] && $dl_config['report_broken'])))
{
	if ($dl_config['report_broken_vc'])
	{
		if ($confirm != 'code')
		{
			...
			$template->assign_vars(array(
				...
				'CONFIRM_CODE' => append_sid("{$phpbb_root_path}downloads.$phpEx", "view=code&amp;code=r"),
		...
		else
		{
			$sql = 'SELECT code FROM ' . DL_HOTLINK_TABLE . '
				WHERE user_id = ' . $user->data['user_id'] . "
					...
					AND hotlink_id = 'repvc'";
			$result = $db->sql_query($sql);
			$saved_code = $db->sql_fetchfield('code');
			$db->sql_freeresult($result);
	...
	*/
	...
	if ($check_status['auth_dl'] && $dl_file['id'])
	{
		...
		*/
		...
		if ($user->data['is_registered'])
		{
			if ($dl_config['overall_traffic'] - $dl_config['remain_traffic'] < $dl_file['file_size'])
			{
				$status = false;
			}
		}
		...
		{
			if ($dl_config['overall_guest_traffic'] - $dl_config['remain_guest_traffic'] < $dl_file['file_size'])
			{
				$status = false;
			}
		}
		...
		*/
		if ($status)
		{
			...
			if ($user->data['is_registered'] && !$dl_file['free'] && !$dl_file['extern']) 
			{
			...
			if (!$dl_file['extern'])
			{
				$remain_traffic = ($user->data['is_registered']) ? 'remain_traffic' : 'remain_guest_traffic';

				$new_remain = $dl_config[$remain_traffic] + $dl_file['file_size'];

				$sql = 'UPDATE ' . DL_CONFIG_TABLE . ' SET ' . $db->sql_build_array('UPDATE', array(
					'config_value' => $new_remain)) . " WHERE config_name = '$remain_traffic'";
				$db->sql_query($sql);

				$sql = 'SELECT cat_traffic_use FROM ' . DL_CAT_TABLE . "
					WHERE id = $cat_id";
				$result = $db->sql_query($sql);
				...
				$sql = 'UPDATE ' . DL_CAT_TABLE . ' SET ' . $db->sql_build_array('UPDATE', array(
					'cat_traffic_use' => $cat_traffic_use)) . " WHERE id = $cat_id";
				$db->sql_query($sql);
			}
		...
		*/
		...
		{
		 	$dl_file_url = $dl_config['dl_path'] . $index[$cat_id]['cat_path'] . $dl_file['real_file'];
			...
			if ($dl_file_size > $max_mem_limit || $dl_config['dl_method'] == 3)
			{
...
else if ($view == 'upload')
{
	...
	if (($dl_config['stop_uploads'] && !$auth->acl_get('a_')) || !sizeof($index) || (!$cat_auth['auth_up'] && !$index[$cat_id]['auth_up'] && !$auth->acl_get('a_')))
	{
		trigger_error('DL_NO_PERMISSION');
	}
...
if (empty($view) && !$inc_module)
{
	...
	if (count($index) > 0)
	{
		foreach(array_keys($index) as $cat_id)
		{
			...
			$config['posts_per_page'] = $dl_config['dl_links_per_page'];
			...
			$last_dl_time = $dl_mod->find_latest_dl($last_dl, (($parent_id) ? $parent_id : $cat_id), $cat_id, array());
	...
	if ($cat)
	{
		...
		if ($total_downloads)
		{
			$pagination = generate_pagination(append_sid("{$phpbb_root_path}downloads.$phpEx?cat=$cat", "sort_by=$sort_by&amp;order=$order"), $total_downloads, $dl_config['dl_links_per_page'], $start, true). '&nbsp;';

			$template->assign_vars(array(
				'PAGINATION' => $pagination,
				'PAGE_NUMBER' => ($total_downloads > $dl_config['dl_links_per_page']) ? sprintf($user->lang['PAGE_OF'], (floor($start / $dl_config['dl_links_per_page']) + 1), ceil( $total_downloads / $dl_config['dl_links_per_page'])) : '')
			);
		}
		...
		if ($physical_size < $dl_config['physical_quota'] && (!$dl_config['stop_uploads']) || ($auth->acl_get('a_') && $user->data['is_registered']))
		{
	...
	{
		$dl_files = array();
		$dl_files = $dl_mod->files($cat, $sql_sort_by, $sql_order, $start, $dl_config['dl_links_per_page'], 'id, description, desc_uid, desc_bitfield, desc_flags, hack_version, extern, file_size, klicks, overall_klicks, rating, long_desc, long_desc_uid, long_desc_bitfield, long_desc_flags');
	...
	$template->assign_vars(array(
		...
		'DL_MODCP' => (isset($total_downloads) && $total_downloads <> 0) ? sprintf($user->lang['DL_MODCP_MOD_AUTH'], '<a href="'.append_sid("{$phpbb_root_path}downloads.$phpEx?view=modcp", "cat_id=$cat").'">', '</a>') : '',
...
$template->assign_vars(array(
	'U_HELP_POPUP' => append_sid("{$phpbb_root_path}dl_help.$phpEx", "help_key="))
);
