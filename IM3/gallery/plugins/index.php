...
* @version $Id: index.php 1334 2009-10-29 21:24:59Z nickvergessen $
...
*/
if (file_exists($phpbb_root_path .  'styles/js/highslide/highslide-full.js'))
{
	$gallery_plugins['plugins'][] = 'highslide';
	$gallery_plugins['slideshow'] = true;
	$template->assign_var('S_GP_HIGHSLIDE', $phpbb_root_path . 'styles/js/highslide/');
}
...
*/
if (file_exists($phpbb_root_path . 'styles/js/lytebox/lytebox.js'))
{
	$gallery_plugins['plugins'][] = 'lytebox';
	$gallery_plugins['slideshow'] = true;
	$template->assign_var('S_GP_LYTEBOX', $phpbb_root_path . 'styles/js/lytebox/');
}
...
*/
if (file_exists($phpbb_root_path . 'styles/js/shadowbox/shadowbox.js'))
{
	$gallery_plugins['plugins'][] = 'shadowbox';
	$gallery_plugins['slideshow'] = true;
	$template->assign_var('S_GP_SHADOWBOX', $phpbb_root_path . 'styles/js/shadowbox/');
}
...
if (!function_exists('generate_image_link_plugins'))
{
	function generate_image_link_plugins($mode)
	{
		...
		switch ($mode)
		{
			...
			break;
			case 'shadowbox':
				$tpl = '<a href="{IMAGE_URL}" title="{IMAGE_NAME}" rel="shadowbox">{CONTENT}</a>';
			break;
			case 'shadowbox_slideshow':
				$tpl = '<a href="{IMAGE_URL}" title="{IMAGE_NAME}" rel="shadowbox;options={slideshowDelay: 3}">{CONTENT}</a>';
			break;
			default:
