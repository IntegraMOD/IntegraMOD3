...
* @file				hacks_list.php 2 2009/01/19 OXPUS
	...
	for ($i = 0; $i < sizeof($dl_files); $i++)
	{
		...
		if ($hacklist[$cat_id])
		{
			...
			$template->assign_block_vars('listrow', array(
				...
				'HACK_AUTHOR_WEBSITE' => ($hack_author_website != '') ? '<a href="' . $hack_author_website . '" target="_blank">' . $user->lang['DL_HACK_AUTOR_WEBSITE'] . '</a>' : '',
				'HACK_DL_URL' => ($hack_dl_url != '') ? '<a href="' . $hack_dl_url . '" target="_blank">' . $user->lang['DL_DOWNLOAD'] . '</a>' : '')
			);
		}
	}
}
