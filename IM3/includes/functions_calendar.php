...
*
* @package calendar
* @version $Id: functions_calendar.$phpEx 2008-07-10 21:16:00Z livewirestu and NeXur $
...
// Day
$weekday_list = array(
    'S'    => $user->lang['CALENDAR']['LONG_DAY'][0],   // Sunday
    'M'    => $user->lang['CALENDAR']['LONG_DAY'][1],   // Monday
    'T'    => $user->lang['CALENDAR']['LONG_DAY'][2],   // Tuesday
    'W'    => $user->lang['CALENDAR']['LONG_DAY'][3],   // Wednesday
    'H'    => $user->lang['CALENDAR']['LONG_DAY'][4],   // Thursday
    'F'    => $user->lang['CALENDAR']['LONG_DAY'][5],   // Friday
    'A'    => $user->lang['CALENDAR']['LONG_DAY'][6],   // Saturday
);

$month_list = array(
    'J'    => $user->lang['CALENDAR']['LONG_MONTH'][1],   // January
    'F'    => $user->lang['CALENDAR']['LONG_MONTH'][2],   // February
    'M'    => $user->lang['CALENDAR']['LONG_MONTH'][3],   // March
    'A'    => $user->lang['CALENDAR']['LONG_MONTH'][4],   // April
    'Y'    => $user->lang['CALENDAR']['LONG_MONTH'][5],   // May
    'U'    => $user->lang['CALENDAR']['LONG_MONTH'][6],   // June
    'L'    => $user->lang['CALENDAR']['LONG_MONTH'][7],   // July
    'G'    => $user->lang['CALENDAR']['LONG_MONTH'][8],   // August
    'S'    => $user->lang['CALENDAR']['LONG_MONTH'][9],   // September
    'O'    => $user->lang['CALENDAR']['LONG_MONTH'][10],   // October
    'N'    => $user->lang['CALENDAR']['LONG_MONTH'][11],   // November
    'D'    => $user->lang['CALENDAR']['LONG_MONTH'][12],   // December
);
...
function generate_month_array($mon, $year, $simple = false)
{
    ...
    else
    {
        ...
        for($dname = 0 ; $dname < 7 ; $dname++)
        {
            ...
            {
                $month[$cal_id . 'dname_' . $dname] = '<b>' . $user->lang['CALENDAR']['DAY_INITIAL'][0] . '</b>';
            }
            else
            {
                $month[$cal_id . 'dname_' . $dname] = $s_show_monday_first == 1 ? '<b>' . $user->lang['CALENDAR']['DAY_INITIAL'][$dname + 1] : '<b>' . $user->lang['CALENDAR']['DAY_INITIAL'][$dname] . '</b>';
            }
        }
...
function get_events($day, $month, $year, $period, $simple = false)
{
    ...
    while($row = $db->sql_fetchrow($result))
    {
		...
		{
			$e_ids[] = substr($row['repeat_id'], 1);
		}
		else if(substr($row['repeat_id'], 0, 1) == 't')
		{
			$t_ids[] = substr($row['repeat_id'], 1);
		}
		else
		{
...
function put_repeat_events($mode, &$event_data)
{
    ...
    $sql = 'INSERT INTO ' . CALENDAR_REPEATS_TABLE . '
        (repeat_id, event_start_time, event_end_time)
        VALUES ' . $event_list;
    $db->sql_query($sql);
}
...
function generate_mini_cal($cal_id)
{
    ...
    $day_initial_langname = $user->lang['CALENDAR']['DAY_INITIAL'];
    $cal_string .= '<tr>';
            ...
            foreach($upcoming_events as $event)
            {
                if(($event['end_time'] > $time_now) && $event['start_time'] < $event_list_end_time)
                {
                    ...
                    $template->assign_block_vars('event_list', array(
                        'TITLE'      => '<a href="'.append_sid("{$phpbb_root_path}calendar.$phpEx", 'mode=view&month=' . $m . '&day=' . $d . '&year=' . $y) . '" title="' . censor_text($event['event_name']) . '">' . censor_text($event['event_name']) . '</a>',
