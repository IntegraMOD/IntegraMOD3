...
function delete_post($forum_id, $topic_id, $post_id, &$data)
{
	...
	$sql = "SELECT config_value FROM " . DL_CONFIG_TABLE . "
		WHERE config_name = 'drop_traffic_postdel'";
	$result = $db->sql_query($sql);
	
	$row = $db->sql_fetchrow($result);
	$drop_traffic = $row['config_value'];
	$db->sql_freeresult($result);
	...
	if ($drop_traffic)
	{
		if ($post_mode == 'delete_topic')
		{
			$sql_traffic = 'newtopic_traffic';
		}
		else
		{
			$sql_traffic = 'reply_traffic';
		}

		$sql = "SELECT config_value FROM " . DL_CONFIG_TABLE . "
			WHERE config_name = '$sql_traffic'";
		$result = $db->sql_query($sql);
		
		$row = $db->sql_fetchrow($result);
		$drop_traffic_amount = $row['config_value'];
		$db->sql_freeresult($result);
