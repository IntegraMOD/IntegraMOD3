					...
					if ($deletemark )
					{
						if (confirm_box(true))
						{
							...
							if ($where_sql)
							{
								...
								$return_url = ($cat_id != 0) ? append_sid("{$phpbb_root_path}mcp.$phpEx", 'i=kb&amp;mode=kb_main&amp;c=' . $cat_id) : append_sid("{$phpbb_root_path}mcp.$phpEx", 'i=kb&amp;mode=kb_main');

								trigger_error($user->lang['ARTICLE_DELETED'] . '<br /><br />' . sprintf($user->lang['RETURN_PAGE'], '<a href="' . $return_url . '">', '</a>'));
							}
						}
						else
						{
...
'i=kb&amp;mode=kb_main&amp;a= ' .$article_id) . '">', '</a>'));
	...
	function list_articles($cat_id)
	{
		...
		$template->assign_vars(array(
			'U_VIEW_CAT'		=> (KB_SEO == true) ? append_sid("{$kb_root_path}categorie-" . $cat_id . '.html') : append_sid("{$kb_root_path}kb_categorie.php", 'id=' . $cat_id),
