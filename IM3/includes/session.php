...
class user extends session
{
  ...
  // end mobile browser detection mod
	...
	// Download MOD 6
		...
		if (!empty($_GET['style']) && $auth->acl_get('a_styles') && !defined('ADMIN_START') || !empty($_GET['style']) && $user->data['user_id'] != ANONYMOUS && !$config['override_user_style'])
		{
...
//--- BEGIN IM3 ---
	/**
	* function:         get_user_im3_config
	* description:      retrieves the user's IntegraMod3 preferences from the db and adds them to the user class
	* parameters:       class $user (passed by reference)
	* returns:          none - modifies user class by reference
	* author:           livewirestu
	* notes:            We use bitfields to enable storage of all user IM3 preferences in a single 32 bit integer
	*/
	function get_user_im3_config()
	{
		global $db;

		$sql = "SELECT user_im3_config
			FROM " . USERS_TABLE . "
				WHERE user_id = " . $this->data['user_id'];
		$result = $db->sql_query($sql);
	    ...
	    {
    		$user_im3_config = $row['user_im3_config'];
	    }
	    else
	    {
    		die("Could not retrieve user info");
		}
	    $db->sql_freeresult($result);

		$this->data['im_show_cal_index'] = $user_im3_config & USER_IM3_SHOW_CAL_INDEX ? 1 : 0;
		$this->data['im_show_shout_index'] = $user_im3_config & USER_IM3_SHOW_SHOUT_INDEX ? 1 : 0;

		// The shift operator at the end (>>) moves the value to the least significant bits of the integer.
		// For example, USER_IM3_EXAMPLE_NUMBER starts at bit 26 so we shift right by 26 bits.
		// If USER_IM3_EXAMPLE_NUMBER is 63 (111111 in binary) it would be stored as
		// 11111100000000000000000000000000 in the database (masked). Shifting right by 26 bits gives:
		// 00000000000000000000000000111111 - Contact livewirestu for a better explanation :)
		$this->data['an_example_number_embedded'] = (($user_im3_config & USER_IM3_EXAMPLE_NUMBER) >> 26);
		$this->data['another_example_number_embedded'] = (($user_im3_config & USER_IM3_ANOTHER_EXAMPLE_NUMBER) >> 20);
		// Debug
		// var_dump($this->data);
	}

	/**
	* function:         set_user_im3_config
	* description:      Stores user's IntegraMod3 preferences to the database
	* parameters:       class $user (passed by reference)
	* returns:          none
	* author:           livewirestu
	* notes:            We use bitfields to enable storage of all user IM3 preferences in a single 32 bit integer
	*/
	function set_user_im3_config()
	{
		...
		$user_im3_config |= ($this->data['im_show_cal_index'] ? USER_IM3_SHOW_CAL_INDEX : 0);
		$user_im3_config |= ($this->data['im_show_shout_index'] ? USER_IM3_SHOW_SHOUT_INDEX : 0);

		$user_im3_config |= (($this->data['an_example_number_embedded'] & 0x1F) << 26);
		$user_im3_config |= (($this->data['another_example_number_embedded'] & 0x1F) << 20);
		$sql = "UPDATE " . USERS_TABLE . "
	        SET user_im3_config = '$user_im3_config'
        		WHERE user_id = " . $this->data['user_id'];
