...
if(!function_exists('process_for_vars'))
{
	function process_for_vars($data)
	{
		...
		if($in)
		{
			...
			$search = array('$portal_version', '$imod_version', '$phpbb_version', '$your_site');
			$replace = array($config['portal_version'], $config['imod_version'], $config['version'], $config['sitename']);
		}
		else
		{
