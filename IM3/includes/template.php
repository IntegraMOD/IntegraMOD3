	...
	function _tpl_load(&$handle)
	{
		...
		else if ($config['load_tplcompile'])
		{
			if(STARGATE)
			{
				...
				// That is, find the correct file and substitute it... Mike
				...
				if (!file_exists($this->files[$handle]))
				{
					...
					//do we check different style types ? this is not added as we may not need it //
					//$style_folder = $user->theme['template_path'];

					// Block file or ordinary file? Notice the use of lock_ and not block_... strpos() ignores the first char?
					if(strpos($missing_filename, 'lock_', 1))
					{
						$alt_filenpath = $phpbb_root_path . 'styles/portal_common/template/blocks/' . $missing_filename;
					}
					else
					{
						$alt_filenpath = $phpbb_root_path . 'styles/portal_common/template/' . $missing_filename;
					}

					// replace file with common file and path...
					$this->files[$handle] = $alt_filenpath;
				}
