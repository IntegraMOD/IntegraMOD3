	...
	function main($id, $mode)
	{
		...
		switch ($mode)
		{
			...
			case 'view':
				...
				// Handle IM3 bitfield settings
				$user->data['im_show_cal_index'] = request_var('show_cal', (bool) $user->data['im_show_cal_index']);
				$user->data['im_show_shout_index'] = request_var('show_shout', (bool) $user->data['im_show_shout_index']);
				...
				$template->assign_vars(array(
					'S_CAL_OVERRIDE'		=> $config['calendar_override_user'] ? true : false,
					'S_CAL_INDEX'			=> $user->data['im_show_cal_index'] ? true : false,
					'S_SHOUT_OVERRIDE'		=> $config['as_override_user'] ? true : false,
					'S_SHOUT_INDEX'			=> $user->data['im_show_shout_index'] ? true : false,
				));
			break;
