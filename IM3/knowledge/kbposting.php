...
* @version $Id: kbposting.php, v 0.2.12 2009/06/06 20:21:36 tas2580 Exp $
...
$data['page_uri'] 			= (KB_SEO == true) ? html_entity_decode(request_var('page_uri', '')) : '';
...
if ($submit == true && $preview == false)
{
	...
	if(KB_SEO == true)
	{
		$sql_where = ($mode == 'edit') ? ' AND article_id != ' . (int) $id : '';
		$sql = 'SELECT page_uri
	...
	if(KB_SEO == true && !empty($data['page_uri']))
	{
		$sql_ary1 = array(
...
if ($mode == 'edit' && ($auth->acl_get('u_edit_kb') || $auth->acl_get('m_edit_kb')) && $preview == false && !sizeof($error))
{
	...
	if ($submit == true && !sizeof($error))
	{
		...
		if(!$mode == 'edit' && $auth->acl_get('m_activate_kb'))
		{
			if(isset($_POST['activ']))
			{
				activate_article($id, $row['user_id']);
			}
			else
			{
				deactivate_article($id, $row['user_id']);
			}
		}
		if ($auth->acl_get('u_attache_kb') && $config['allow_attachments'])
		{
...
if($kb_config['activ_diff'])
{
	...
	if($mode == 'del_diff')
	{
		if (confirm_box(true))
		{
			...
			$db->sql_query($sql);
			trigger_error($user->lang['ARTICLE_DELETED'] . '<br /><br />' . sprintf($user->lang['BACK_TO_POSTING'], '<a href="' . append_sid("{$kb_root_path}kbposting.$phpEx", 'mode=edit&amp;id=' . $id) . '">', '</a>') . '<br /><br />' . sprintf($user->lang['BACK_TO_KB'], '<a href="' . append_sid("{$kb_root_path}") . '">', '</a>'));
		}
		else
		{
	...
	if($mode == 'restore' && $auth->acl_get('u_restore_kb'))
	{
		if (confirm_box(true))
		{
			...
			$db->sql_query('UPDATE ' . KB_ARTICLE_TABLE . ' SET ' . $db->sql_build_array('UPDATE', $sql_ary) . ' WHERE article_id = ' . (int) $id);
			article_log($id, $user->lang['ARTICLE_RESTORED']);
			trigger_error($user->lang['ARTICLE_RESTORED'] . '<br /><br />' . sprintf($user->lang['BACK_TO_POSTING'], '<a href="' . append_sid("{$kb_root_path}kbposting.$phpEx", 'mode=edit&amp;id=' . $id) . '">', '</a>') . '<br /><br />' . sprintf($user->lang['BACK_TO_KB'], '<a href="' . append_sid("{$kb_root_path}") . '">', '</a>'));
	...
	$sql = 'SELECT d.diff_id, d.time, d.edit_reason, u.username, u.user_colour, u.user_id
		...
		WHERE d.user_id = u.user_id
			AND d.article_id = ' . (int) $id . '
		ORDER BY d.time DESC';
	$result = $db->sql_query($sql);
	while ($row = $db->sql_fetchrow($result))
	{
		$template->assign_block_vars('diff_row', array(
			'U_DIFF'		=> append_sid("{$kb_root_path}kbposting.$phpEx", 'mode=edit&amp;id=' . $id . '&amp;diff_id=' . $row['diff_id']) . '#diff',
			'TIME'			=> $user->format_date($row['time']),
			'EDIT_REASON'	=> $row['edit_reason'],
			'USER'			=> get_username_string('full', $row['user_id'], $row['username'], $row['user_colour']),
			'U_DELETE'		=> append_sid("{$kb_root_path}kbposting.$phpEx", 'mode=del_diff&amp;id=' . $id . '&amp;diff_id=' . $row['diff_id']),
			'U_RESTORE'		=> append_sid("{$kb_root_path}kbposting.$phpEx", 'mode=restore&amp;id=' . $id . '&amp;diff_id=' . $row['diff_id']),
		));
	}
...
$template->assign_vars(array(
	...
	'S_RESTORE_ARTICLE'			=> $auth->acl_get('u_restore_kb'),
