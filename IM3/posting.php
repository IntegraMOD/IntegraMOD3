...
//---END CALENDAR MOD---
    ...
    {
      $message_parser->message .= "\n\n[size=80][b][i][ Post made via Mobile Device ][/i][/b][/size] <img src='images/mobile.png' alt='mobile post'/>";
    }
...
//---BEGIN CALENDAR MOD---
            // Can't be any calendar data if this is not the first post of the topic
            if($data['topic_first_post_id'] == $post_id)
            {
                if($event_data_valid && $is_event)
                {
                    $data = array_merge($data, array(
                        // CALENDAR - Add stuff to db array
                        'topic_calendar_time'       => str_to_unix($post_data['event_start_time'], $post_data['event_start_date']) - $user->timezone - $user->dst,
                        'topic_calendar_duration'   => str_to_unix($post_data['event_end_time'], $post_data['event_end_date']) - str_to_unix($post_data['event_start_time'], $post_data['event_start_date']),
                        'event_repeat'              => $post_data['event_repeat'],
                        'invite_attendees'          => $post_data['invite_attendees'],
                        'event_attendees'           => !empty($post_data['event_attendees']) ? $post_data['event_attendees'] : '',
                        'event_non_attendees'       => !empty($post_data['event_non_attendees']) ? $post_data['event_non_attendees'] : '',
...
//---BEGIN CALENDAR MOD---
$template->assign_vars(array(
    'DATE_IN'                               => isset($post_data['event_start_date']) ? $post_data['event_start_date'] : '',
    'TIME_IN'                               => isset($post_data['event_start_time']) ? $post_data['event_start_time'] : '',
    'DATE_OUT'                              => isset($post_data['event_end_date']) ? $post_data['event_end_date'] : '',
    'TIME_OUT'                              => isset($post_data['event_end_time']) ? $post_data['event_end_time'] : '',
    'S_EVENT_REPEAT'                        => !empty($post_data['event_repeat']) ? true : false,
    'REPEAT_COUNT_SELECTED'                 => !empty($post_data['event_repeat_count']) ? $post_data['event_repeat_count'] : '',
    'REPEAT_PERIOD_SELECTED'                => !empty($post_data['event_repeat_when']) ? $post_data['event_repeat_when'] : '',
    'REPEAT_NTH_DAY_POSITION_SELECTED'      => !empty($post_data['repeat_nth_pos']) ? $post_data['repeat_nth_pos'] : '',
    'REPEAT_NTH_DAY_SELECTED'               => !empty($post_data['repeat_nth_weekday']) ? $post_data['repeat_nth_weekday'] : '',
    'REPEAT_NTH_COUNT_SELECTED'             => !empty($post_data['repeat_nth_count']) ? $post_data['repeat_nth_count'] : '',
    'REPEAT_NTH_MONTH_POSITION_SELECTED'    => !empty($post_data['nth_month_position']) ? $post_data['nth_month_position'] : '',
    'REPEAT_NTH_MONTH_DAY_SELECTED'         => !empty($post_data['nth_month_weekday']) ? $post_data['nth_month_weekday'] : '',
    'REPEAT_NTH_MONTH_SELECTED'             => !empty($post_data['nth_month_month']) ? $post_data['nth_month_month'] : '',
    'S_INVITE_ATTENDEES'                    => isset($post_data['invite_attendees']) ? $post_data['invite_attendees'] : 0,
));
//---END CALENDAR MOD---
    ...
    && $auth->acl_get('u_new_event'))
{
    ...
    $poster_id = (isset($post_data['topic_poster']) ? $post_data['topic_poster'] : $user->data['user_id']);
    $user_groups = get_users_groups($poster_id);

    $posted_groups = empty($post_data['event_groups']) ? array() : explode(';', $post_data['event_groups']);
    ...
    $template->assign_vars(array(
        'S_SHOW_EVENT_BOX'  => true,
        'S_POSTING_TOPIC'   => true,
        'S_SGP_AJAX'        => true,
    ));
...
//---BEGIN CALENDAR MOD---
function handle_topic_event_delete($forum_id, $topic_id, &$post_data)
{
    global $user, $db, $auth;
    global $phpbb_root_path, $phpEx;
