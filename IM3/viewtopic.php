...
$site_page = urlencode(($config['cookie_secure'] ? "https://" : "http://").$_SERVER['HTTP_HOST'].$_SERVER['PHP_SELF']."?f=" . $forum_id . "&t=" .$topic_id);
...
'SITE_URL'      => $site_page,
	...
	'QUICK_REPLY_IMG'	=> $user->img('button_topic_qreply', 'QUICK_REPLY'),
	'ALLOW_QUICK_REPLY'	=> $user->data['user_id'] == ANONYMOUS ? false : true,
...
'ALLOW_DIG'         => $user->data['user_id'] == ANONYMOUS ? false : true,
    'DIG_IMG'           => $user->img('dig', 'Digg It'),
    'ALLOW_DEL'         => $user->data['user_id'] == ANONYMOUS ? false : true,
    'DEL_IMG'           => $user->img('del', 'Delicious!'),
    'ALLOW_SLASHDOT'    => $user->data['user_id'] == ANONYMOUS ? false : true,
    'SLASHDOT_IMG'      => $user->img('slashdot', 'Slash Dot'),
    'ALLOW_RDS'         => $user->data['user_id'] == ANONYMOUS ? false : true,
    'RDS_IMG'           => $user->img('rds', 'Yahoo My Web'),
    'ALLOW_SPURL'       => $user->data['user_id'] == ANONYMOUS ? false : true,
    'SPURL_IMG'         => $user->img('spurl', 'Spurl'),
    'ALLOW_LINKAGOGO'   => $user->data['user_id'] == ANONYMOUS ? false : true,
    'LINKAGOGO_IMG'     => $user->img('linkagogo', 'Link a GoGo'),
    'ALLOW_REDDIT'      => $user->data['user_id'] == ANONYMOUS ? false : true,
    'REDDIT_IMG'        => $user->img('reddit', 'Redd It'),
    'ALLOW_TECH'        => $user->data['user_id'] == ANONYMOUS ? false : true,
    'TECH_IMG'          => $user->img('tech', 'Technorati'),
    'ALLOW_GOOGLE'      => $user->data['user_id'] == ANONYMOUS ? false : true,
    'GOOGLE_IMG'        => $user->img('google', 'Google'),
    'ALLOW_PROP'        => $user->data['user_id'] == ANONYMOUS ? false : true,
    'PROP_IMG'          => $user->img('prop', 'Propeller'),
    'ALLOW_NETV'        => $user->data['user_id'] == ANONYMOUS ? false : true,
    'NETV_IMG'          => $user->img('netv', 'Netvouz'),
    'ALLOW_STUMBLE'     => $user->data['user_id'] == ANONYMOUS ? false : true,
    'STUMBLE_IMG'       => $user->img('stumble', 'Stumbleupon'),
   
    'S_DISPLAY_DIG'       => ($auth->acl_get('u_dig') && $config['allow_dig']) ? true : false,
    'S_DISPLAY_DEL'       => ($auth->acl_get('u_del') && $config['allow_del']) ? true : false,
    'S_DISPLAY_SLASHDOT'  => ($auth->acl_get('u_slashdot') && $config['allow_slashdot']) ? true : false,
    'S_DISPLAY_RDS'       => ($auth->acl_get('u_rds') && $config['allow_rds']) ? true : false,
    'S_DISPLAY_SPURL'     => ($auth->acl_get('u_spurl') && $config['allow_spurl']) ? true : false,
    'S_DISPLAY_LINKAGOGO' => ($auth->acl_get('u_linkagogo') && $config['allow_linkagogo']) ? true : false,
    'S_DISPLAY_REDDIT'    => ($auth->acl_get('u_reddit') && $config['allow_reddit']) ? true : false,
    'S_DISPLAY_TECH'      => ($auth->acl_get('u_tech') && $config['allow_tech']) ? true : false,
    'S_DISPLAY_GOOGLE'    => ($auth->acl_get('u_google') && $config['allow_google']) ? true : false,
    'S_DISPLAY_PROP'      => ($auth->acl_get('u_prop') && $config['allow_prop']) ? true : false,
    'S_DISPLAY_NETV'      => ($auth->acl_get('u_netv') && $config['allow_netv']) ? true : false,
    'S_DISPLAY_STUMBLE'   => ($auth->acl_get('u_stumble') && $config['allow_stumble']) ? true : false,
...
$db->sql_query($sql);
	...
	$postrow = array(
		...
		'POSTER_FLAG'		=> '<img src="' . (sgp_get_user_country_flag($topic_data['topic_poster'])). '" alt="L_FLAG" />',
		'POST_ID'			=> $row['post_id'],
		'POSTER_ID'			=> $poster_id,
		...
		'POSTER_FLAG'		=> '<img src="' . (sgp_get_user_country_flag($poster_id)). '" alt="Flag" />',
	);
...
//---BEGIN CALENDAR MOD---
if(!empty($topic_data['topic_calendar_time']) && !empty($topic_data['topic_calendar_duration']))
{
    ...
    $sql = 'SELECT username, user_id, user_colour FROM ' . USERS_TABLE . '
            WHERE ' . $db->sql_in_set('user_id', explode(',', $topic_data['event_attendees']));
    $result = $db->sql_query($sql);
    ...
    $sql = 'SELECT username, user_id, user_colour FROM ' . USERS_TABLE . '
            WHERE ' . $db->sql_in_set('user_id', explode(',', $topic_data['event_non_attendees']));
    $result = $db->sql_query($sql);
    ...
    // Get the forum_id and topic_id from the link.
    $fid = $topic_data['forum_id'];
    $tid = $topic_data['topic_id'];
    $reg_link = "fid=$fid&amp;tid=$tid";
    
    if(!empty($topic_data['event_repeat']))
    {
        $topic_data['repeats'] = array();
        $sql = "SELECT event_start_time FROM " . CALENDAR_REPEATS_TABLE . "
            WHERE repeat_id = 't$tid'
            ORDER BY event_start_time ASC";
        $result = $db->sql_query($sql);
        while($reps = $db->sql_fetchrow($result))
        {
            $topic_data['repeats'][] = $reps['event_start_time'];
        }
        $db->sql_freeresult($result);                    
    }
    
    $template->assign_vars(array(
        'START_TIME'            => gmdate($user->date_format, ($topic_data['topic_calendar_time'] + $user->timezone + $user->dst)),
        'END_TIME'              => gmdate($user->date_format, ($topic_data['topic_calendar_time'] + $topic_data['topic_calendar_duration'] + $user->timezone + $user->dst)),
        'DURATION'              => calc_duration(0, $topic_data['topic_calendar_duration']),
        'S_HAS_EVENT'           => true,
        'S_HAS_REPEATS'         => $topic_data['event_repeat'] ? true : false,
        'REPEAT_LIST'           => isset($topic_data['repeats']) ? generate_repeat_link_list($topic_data['repeats']) : '',
        'U_ASSOCIATED_EVENT'    => 'calendar.php?mode=view&month=' . $date['mon'] . '&day=' . $date['mday'] . '&year=' . $date['year'],
        'S_SHOW_ATTENDEES_LIST' => true,
        'S_IS_SIGNED_UP'        => $s_is_signed_up,
        'S_IS_SIGNED_DOWN'      => $s_is_signed_down,
        'S_IS_INVITED'          => $topic_data['invite_attendees'] ? true : false,
        'SIGNED_UP_LIST'        => empty($signed_up_list) ? null : $signed_up_list,
        'SIGNED_DOWN_LIST'      => empty($signed_down_list) ? null : $signed_down_list,
        'U_SIGN_UP'             => append_sid($phpbb_root_path . 'calendar.' . $phpEx, "mode=attend&amp;opt=accept&amp;fid=$fid&amp;tid=$tid"), 
        'U_SIGN_DOWN'           => append_sid($phpbb_root_path . 'calendar.' . $phpEx, "mode=attend&amp;opt=decline&amp;fid=$fid&amp;tid=$tid"),
        'U_UNREG'               => append_sid($phpbb_root_path . 'calendar.' . $phpEx, "mode=attend&amp;opt=unreg&amp;fid=$fid&amp;tid=$tid"),
    ));
}
//---END CALENDAR MOD---
